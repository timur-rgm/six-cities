...
export default function Review(props: ReviewType): JSX.Element {
  const {reviews} = props;
  const [inputText, setInputText] = useState('');
  const textareaElement = document.querySelector('#review');

  return(
    <section className="property__reviews reviews">
      <h2 className="reviews__title">Reviews &middot; <span className="reviews__amount">1</span></h2>
      <ul className="reviews__list">
        {reviews.map(({avatarSrc, userName, reviewRate, reviewDate, reviewText}, i) => {
          return (
